...
EXPECTED_TRACE_COUNT = 5000
EXPECTED_TRACE_LEN = 84000
...
WINDOWS = {
    "1": {
        ...
        "desc": "前 17000 个采样点（0..16999），默认视为与 ciphertext/input 相关的第10轮轮密钥泄漏",
    },
    "2": {
        "name": "tail17k",
        "label": "尾区间",
        ...
        "desc": "末 17000 个采样点，默认视为与 plaintext/output 相关的第0轮轮密钥泄漏",
    },
}
...
def expand_master_key_to_round_keys(master_key: bytes) -> list[bytes]:
    if len(master_key) != 16:
        raise ValueError("MASTER_KEY 必须为 16 字节")
    ...
    return [bytes(b for word in words[r * 4:(r + 1) * 4] for b in word) for r in range(11)]


def get_expected_window_keys() -> dict[str, bytes]:
    round_keys = expand_master_key_to_round_keys(MASTER_KEY)
    return {
        "1": round_keys[10],
        "2": round_keys[0],
    }
...
def prompt_h5_path() -> RunPaths:
    while True:
        raw = input("请输入 HDF5 文件绝对路径: ").strip()
        if not raw:
            print("路径不能为空。")
            continue
...
def load_h5_metadata_and_io(h5_path: Path) -> tuple[np.ndarray, np.ndarray, int, int]:
    with h5py.File(h5_path, "r") as h5_file:
        required = ["ciphertext", "plaintext", "traces"]
        missing = [name for name in required if name not in h5_file]
        if missing:
            raise KeyError(f"HDF5 缺少必要 Dataset: {missing}")

        ciphertext = np.asarray(h5_file["ciphertext"][...], dtype=np.uint8)
        plaintext = np.asarray(h5_file["plaintext"][...], dtype=np.uint8)
        traces_ds = h5_file["traces"]

        if traces_ds.ndim != 2:
            raise ValueError(f"traces 必须是二维数组，实际 ndim={traces_ds.ndim}")

        num_traces, total_samples = traces_ds.shape

    if ciphertext.shape != (num_traces, 16):
        raise ValueError(f"ciphertext 形状必须为 ({num_traces}, 16)，实际 {ciphertext.shape}")
    if plaintext.shape != (num_traces, 16):
        raise ValueError(f"plaintext 形状必须为 ({num_traces}, 16)，实际 {plaintext.shape}")

    if num_traces != EXPECTED_TRACE_COUNT:
        raise ValueError(f"trace 条数必须为 {EXPECTED_TRACE_COUNT}，实际 {num_traces}")
    if total_samples != EXPECTED_TRACE_LEN:
        raise ValueError(f"单条 trace 采样点必须为 {EXPECTED_TRACE_LEN}，实际 {total_samples}")

    return ciphertext, plaintext, num_traces, total_samples


def build_window_bounds(window_key: str, total_samples: int) -> tuple[int, int]:
    spec = WINDOWS[window_key]

    if "tail_len" in spec:
        tail_len = int(spec["tail_len"])
        start = total_samples - tail_len
        end = total_samples - 1
    else:
    ...
    start = max(0, start)
    end = min(total_samples - 1, end)

    if start > end:
        raise ValueError(f"{spec['label']} 在当前迹线长度下为空")

    return start, end


def build_window_points(window_key: str, total_samples: int, step: int) -> np.ndarray:
    start, end = build_window_bounds(window_key, total_samples)
...
def cache_file_paths(
    ...
    stem = "".join(ch if ch.isalnum() or ch in "._-" else "_" for ch in h5_path.stem)
    tag = f"{stem}_{name}_n{num_traces}_len{total_samples}_step{step}_pts{num_points}"
...
def load_or_build_window_cache(
    ...
    cache_npy, cache_meta = cache_file_paths(cache_dir, h5_path, window_key, num_traces, total_samples, step, int(points.size))
    ...
    if cache_npy.exists() and cache_meta.exists():
        try:
            ...
            valid = (
                ...
                and meta.get("num_traces") == num_traces
                ...
                and meta.get("h5_path") == str(h5_path)
            )
            if valid:
    ...
    traces = np.empty((num_traces, points.size), dtype=np.float32)

    with h5py.File(h5_path, "r") as h5_file:
        traces_ds = h5_file["traces"]
        for i in range(num_traces):
            traces[i, :] = traces_ds[i, start:end + 1:step]
            if (i + 1) % 200 == 0 or (i + 1) == num_traces:
                print(f"    已加载 {i + 1}/{num_traces} 条")
...
def build_hit_report(window_key: str, results: list[ByteAttackResult]) -> dict[str, Any]:
    expected = get_expected_window_keys().get(window_key)
    if expected is None:
...
def choose_windows() -> list[str]:
    print("选择兴趣区间：")
    print("  1 -> 首区间: 前 17000 点，默认对应 ciphertext/input 侧")
    print("  2 -> 尾区间: 末 17000 点，默认对应 plaintext/output 侧")
...
def main() -> None:
    run_paths = prompt_h5_path()
    ensure_dirs(run_paths)

    print("=" * 56)
    print("AES 解密 CPA 工具 V2（HDF5 单文件输入版）")
    print("=" * 56)
    print("[*] 保留原有 CPA 逻辑，仅把输入源改为单一 HDF5 文件")
    print("[*] HDF5 必须包含 3 个 Dataset: ciphertext, plaintext, traces")
    print(f"[*] 预期 trace 条数: {EXPECTED_TRACE_COUNT}")
    print(f"[*] 预期单条迹线长度: {EXPECTED_TRACE_LEN}")
    print("[*] 首区间模型: bit(InvSBox(C ^ K10))，ROI=前 17000 点")
    print("[*] 尾区间会自动跑两种模型: bit(SBox(P ^ K0)) 与 bit(P ^ K0)，ROI=末 17000 点")
    print("[*] 每个 key 对 bit0..bit7 分别做 CPA，再按 8 个 bit 峰值均值的 Top10 排名输出")
    print()
    print(f"[*] HDF5 文件: {run_paths.h5_path}")
    ...
    print()
    print("[+] 读取 HDF5 元数据与输入输出对...")
    ciphertexts, plaintexts, num_traces, total_samples = load_h5_metadata_and_io(run_paths.h5_path)

    print(f"[+] 使用迹线数: {num_traces}")
    print(f"[+] 单条迹线长度: {total_samples}")

    run_tag = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{run_paths.h5_path.stem}"
    all_master_candidates: dict[str, str] = {}

    for window_key in window_keys:
        print()
        print(f"[+] 准备 {WINDOWS[window_key]['label']} ...")
        cache_path, points = load_or_build_window_cache(
            h5_path=run_paths.h5_path,
    ...
    if len(all_master_candidates) == 2:
        mk1 = all_master_candidates.get("1:sbox_input")
        mk2 = all_master_candidates.get("2:ark_output")
        print()
        print("=" * 56)
        print("跨区间主密钥一致性检查")
        print("=" * 56)
